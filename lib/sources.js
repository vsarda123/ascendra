...
//
// Attribution comes from utm_campaign first, which carries the numeric Meta
// campaign ID and so names the campaign exactly. utm_medium is the fallback
// and is far messier ("paid", "Paid", "paid_social", "DR Email", blank on 43
// of 322 rows), so it resolves only the values named in
// UTM_MEDIUM_CAMPAIGN_MAP and otherwise leaves the booking unattributed.
async function fetchBookings(campaignIdToName = {}) {
  ...
  rows.slice(1).forEach((r, i) => {
    ...
    const decodedCampaign = utmCampaign.replace(/\+/g, ' ');

    // Campaign ID first (exact and rename-proof), then a hand-written
    // campaign name, then the medium.
    const campaign =
      campaignIdToName[utmCampaign] ||
      aliasMap[decodedCampaign.toLowerCase()] ||
      mediumMap[utmMedium.toLowerCase()] ||
      null;
    ...
    const isOrganic = organic.has(utmMedium.toLowerCase()) || organic.has(utmSource.toLowerCase());
    const channel = campaign ? 'paid-attributed'
      : isOrganic ? 'organic'
      : (utmMedium || utmCampaign) ? 'paid-unattributed'
      : 'unknown';
    ...
    bookings.push({
      ...
      utmCampaign: decodedCampaign || null,
...
module.exports = {
  ...
  fetchSheetLeads, fetchBookings, discoverClickFunnelsPages,
  mergeDailySpend,
};
