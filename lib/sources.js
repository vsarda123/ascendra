...
const META_INSIGHTS_FIELDS = [
  ...
  // that paid for it.
  'campaign_id', 'campaign_name', 'adset_name', 'ad_name', 'date_start',
...
async function fetchBookings() {
  ...
  rows.slice(1).forEach((r, i) => {
    ...
    bookings.push({
      ...
      generatedDate,
      generatedHour,
      // campaign and channel are filled in by resolveBookingAttribution once
      // Meta's campaign list is available; utm_campaign carries the raw
      // value (usually a numeric Meta campaign ID) needed to do that.
      campaign: null,
      channel: null,
...
function resolveBookingAttribution(bookings, campaignIdToName = {}) {
  ...
  return bookings.map(b => {
    ...
    return { ...b, campaign, channel, landingPage: pageMeta ? pageMeta.landingPage : null };
  });
}
